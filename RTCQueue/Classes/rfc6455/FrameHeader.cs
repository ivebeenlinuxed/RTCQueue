...
namespace RTCQueue.WebSocket.rfc6455
{
    internal class FrameHeader
    {
        ...
        public byte[] ToBytes(bool IsByte = false)
        {
			...
			byte[] b = new byte[0];
			foreach (byte[] seg in headerBytes) {
				int initial = b.Length;
				Array.Resize<byte>(ref b, b.Length+seg.Length);
				Array.Copy(seg, 0, b, initial, seg.Length); 
			}
			return b;
